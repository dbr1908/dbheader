...
do
	tput civis
	echo -ne "\033[34m\r[*] Ap shiphang dang Download kiba donkam...\e[33m[\033[32m$i\033[33m]\033[0m   ";
	sleep $delay
...
bash ~/dbheader/.banner.sh ${col} ${TNAME}
echo "";
...
tput sgr 0
if [[ ${PROC32} == [Y/y] ]]; then
	if [ -e $HOME/db-header.txt ]; then
		rm $HOME/db-header.txt;
	fi
